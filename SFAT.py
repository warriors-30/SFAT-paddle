...
if __name__ == '__main__':
    ...
    if args.agg_opt == 'Scaffold':
        c_global_model=copy.deepcopy(global_model).to(device)
        nets = []
        for i in range(args.num_users):
            net = copy.deepcopy(c_global_model).to(device)
            nets.append(net)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if args.agg_opt == 'Scaffold':
            ...
            for key in total_delta:
                total_delta[key] = 0.0*total_delta[key]
        ctr = 0
        for idx in idxs_users:
            ...
            if args.agg_opt == 'Scaffold':
                w, loss, ide, idx_train, c_local_mdoel, c_delta_para, pp_index = local_model.update_weights_scaffold(
        ...
        if args.agg_opt == 'Scaffold':
            ...
            for key in c_global_para:
                if 'int' in str(c_global_para[key].dtype):
                    c_global_para[key] += paddle.cast(total_delta[key],dtype='int')
                else:
                    c_global_para[key] += total_delta[key]
            c_global_model.set_state_dict(c_global_para)
        ...
        # update global weights
        global_model.set_state_dict(global_weights)
